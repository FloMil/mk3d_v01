      ...
      do i=1,vgrid(n,m)%nr
         do j=1,vgrid(n,m)%nlat
            do k=1,vgrid(n,m)%nlong
               ...
               IF (vgrid(n,m)%velocity(i,j,k) .LT. 1) THEN
                  WRITE(*,*) '* * * values are the following :  * * *'
                  WRITE(*,*) 'type     : ',m
                  WRITE(*,*) 'velocity : ',vgrid(n,m)%velocity(i,j,k)
                  WRITE(*,*) 'position : ',i,j,k
                  STOP
               ENDIF
            end do
         end do
...
!
 subroutine write_arrivaltime_grid(src,path)
  ...
  do i3=1,pgrid%nlong
     do i2=1,pgrid%nlat
        do i1=1,pgrid%nr
           !write(19,'(3i6,f12.5)') i3, i2, i1, pgrid%arrivaltime(i1,i2,i3)
