...
! MODIFIED
!----------------------------
  CHARACTER(5)  :: outfilename
  CHARACTER(19) :: outfilefile
!----------------------------
! MODIFIED
...
!  print *,'nreceivers is', n_receivers
  ...
  do ns=1,n_sources
     ...
     if (is_teleseismic == 1) then
        ...
        read (*,*) s%teleseismic_phase
        read (*,*) s%r,s%lat,s%long
...
! read in the required paths (reflections/transmissions)
     ...
     read(*,*) s%n_paths    ! the number of paths to this source
...
!        print *,'ntf',s%path(n)%n_tf
        ...
        read (*,*) s%path(n)%sequence(1:2*n_tf)
        read (*,*) s%path(n)%vtype_sequence(1:n_tf)
...
! read the information about which paths are to be saved from the
! file gridsave.in
...
!----------------------------
   ...
   READ(*,*) outfilename
   IF (is_teleseismic .EQ. 1) outfilefile='data/srcs/' // outfilename // '.dat'
   IF (is_teleseismic .EQ. 0) outfilefile='data/recs/' // outfilename // '.dat'
   OPEN(19,file=outfilefile) 
!----------------------------
! MODIFIED
